...
svm.tune <- tune(svm,
                 ...
                 data = titanic,
                 # kernel="radial",
                 # ranges=list(cost=10^(-1:2), gamma=c(.5,1,1.5,2)))
                 kernel="polynomial",
                 ranges=list(degree=(1:3), gamma=c(1,3,9,13)))
print(svm.tune)
...
rmse <- sqrt( mean( (as.numeric(test_classes) - as.numeric(pred_svm))^2 , na.rm = TRUE ) )
rmse

confusion_matrix <- table(pred_svm,test_classes)
...
## 
titanic_validation$Survived <- validation_pred_svm
...
# write.csv(export, "# Competition/export/svm_radial_features(0.4239992_0.8202247).csv", row.names=FALSE)
# write.csv(export, "# Competition/export/svm_radial_features(0.2998127_0.9101124).csv", row.names=FALSE)
write.csv(export, "# Competition/export/svm_pol_features(25_2054).csv", row.names=FALSE)